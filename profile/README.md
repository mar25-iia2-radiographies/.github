# 🩻 mar25-iia2-radiographies

**Analyse IA des radiographies COVID-19 basée sur le dataset Kaggle officiel (21 165 images)**[1]

## 🎯 Dataset Principal
[![Kaggle COVID19 Radiography](https://img.shields.io/badge/Kaggle-COVID19%20Radiography-blueviolet?logo=kaggle&logoColor=whitehttps://www.kaggle.com/datasets/tawsifurrahman/covid19-radiography-databasehttps://www.kaggle.com/datasets/tawsifurrahman/covid19-radiography-database)  
**Images :** 21 165 radiographies thoraciques  
**Classes :** COVID, Lung Opacity, Normal, Viral Pneumonia[1]

## 📂 Nos Projets

- **[Modèle CNN Classification](https://github.com/mar25-iia2-radiographies/covid-classification)**  
  Classification multi-classe (COVID/Normal/Pneumonie) - **Remplacez par votre repo**


## 📊 Métriques Projet
```
[![Python](https://img.shields.io/badge/Python-3.10+-blue)](https://python.org)
[![PyTorch](https://img.shields.io/badge/PyTorch-2.1-orange)](https://pytorch.org)
[![Dataset](https://img.shields.io/badge/Dataset-21k%20images-green)](https://kaggle.com/datasets/tawsifurrahman/covid19-radiography-database)
```

## 👥 Équipe mar25-iia2
- **Romcro** - ML Engineer ([@Romcro](https://github.com/Romcro))
- **Romcro** - ML Engineer ([@Romcro](https://github.com/Romcro))
- **Romcro** - ML Engineer ([@Romcro](https://github.com/Romcro))
- **Romcro** - ML Engineer ([@Romcro](https://github.com/Romcro))
- **Étudiants IIA2** - Data Scientists

## 🚀 Quick Start
```bash
# Clonez le dataset Kaggle
kaggle datasets download -d tawsifurrahman/covid19-radiography-database

# Voir tous nos repos
https://github.com/orgs/mar25-iia2-radiographies/repositories
```
